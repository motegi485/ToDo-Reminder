...
| 全部完了の状態から子のチェックを外した | **自動完了を取り消して未完了へ戻す**（自動完了のちょうど逆操作）。繰り返しタスクなら完了ログも対にして取り下げる |
...
| 完了済みタスクを編集して未完了の子が生じた | 未完了へ戻す（完了済み定量タスクの現在値を目標未満へ編集したときと対称） |
...
達成の単位ではないため、レポートのストリーク・週間達成率・月次バーの母集団は変わりません。
