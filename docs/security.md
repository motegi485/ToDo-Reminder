...
端末追加の手入力を減らすため、設定画面に QR の表示と読み取りがあります。**QR は「読める形にした credential」です。**

| 判断 | 内容 |
|---|---|
| QR に載せる値 | **同期コードの生文字列 12 文字だけ。** アプリの URL は載せない |
| 理由 | URL に載せると同期コードがブラウザ履歴、共有シート、スクリーンショット、Referer、Pages のアクセスログに残りうる。上表の「bearer credential として扱う」方針と衝突するため採らなかった |
| 代償 | OS 標準のカメラアプリでは「ただの文字列」にしか見えない。読み取りは**アプリ内のカメラ機能でのみ**成立する |
| 読み取り手段 | `jsqr` によるアプリ内デコード。`BarcodeDetector` は Safari (iOS/macOS) で既定無効のため採らなかった（caniuse / MDN BCD を 2026-08-16 に確認） |
| カメラ映像 | 端末内の `<canvas>` で処理するだけで送信しない。権限は読み取り画面を開いたときだけ要求し、閉じたらトラックを停止する |
| 実行経路 | 読み取りは入力欄を埋めるだけ。切り替えは手入力と同じ確認ダイアログ → `switchSyncCode()` を通る |

残る露出は表示 UI では消せません。QR はボタンを押したときだけ表示し、「このコードを知る人はデータを読み書きできる」「一度渡すと取り消せない」ことを画面に併記していますが、**カフェや職場で開けば肩越しに読まれうること、スクリーンショットを技術的に止められないことは変わりません。** 同期コードには失効・再発行の機能がないため（下表）、渡した後に取り消す手段もありません。

この機能のために `qrcode-generator`（MIT、生成）と `jsqr`（Apache-2.0、読み取り）をブラウザ成果物へ追加しています。どちらも依存ゼロで、CDN からは読み込みません（オフラインで動く必要があるため）。`jsqr` は最終公開が 2021-04-24 で実質的に更新が止まっています（[known-limitations.md](./known-limitations.md)）。
...
`npm run dev` は **localhost にだけ**待ち受けます（2026-08-31 に変更）。このプロジェクトの通常手順は、リポジトリ root 直下に Git 管理外の `.env` と `wrangler.toml` を置きます。dev サーバーを全インターフェースへ公開すると、同一 LAN の第三者に API URL、運用設定、同期コード allowlist が晒される経路になり得ます。`server.fs.deny` にもこれらを追加してありますが、**deny を列挙すると Vite の既定値は置き換わる**ので、既定（`.env`, `.env.*`, `*.{crt,pem}`）も `vite.config.ts` に明示しています。

実機（スマートフォン）確認で LAN アクセスが必要なときだけ `npm run dev:lan`（= `vite --host`）を使い、信頼できるネットワークで実行してください。
...
| QR の肩越し露出 | ボタン起動と注意書きで緩和するが、表示中に読まれる可能性とスクリーンショットは技術的に止められない。 |
