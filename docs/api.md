...
`subtasks`（タスク内のチェックリスト）も同じ素通しの列です。`recurrence_rule` と同じく、**リクエスト／レスポンスでは配列**、D1 では JSON 文字列として保存されます。サーバーが見るのは次の 2 点だけで、要素の中身（`id` / `title` / `done`）は検証しません。

| 検証 | 理由 |
|---|---|
| 配列であること | 配列以外を通すと、pull した全端末がその値をそのまま保存し、カードの描画が壊れる |
| JSON 化した長さが `SUBTASKS_MAX_BYTES` 以内であること | 上限が無いと 1 リクエストで大量に書き込める（`title` と同じ理屈） |

要素の形を固定しないのは、クライアントが子にフィールドを 1 つ足した瞬間に、サブタスクを持つタスクだけがサイレントに同期されなくなるためです。空配列は `invalid` にしません（サーバーは意味を解釈しないため）。「サブタスクなし」へ畳むのはクライアント側の役目です。
