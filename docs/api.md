...
| 本文サイズ | バイト上限を超えると `413`。**判定は同期コードの検証と認可より前**（許可されていない相手に認可前の巨大パースをさせないため）。上限は `/api/sync/push` が 4 MiB、他の 3 API が 32 KiB。`Content-Length` は省略・詐称できるため、実バイト数で打ち切る。 |
...
**列は「キーの有無」で意味が変わります**（[invariants.md I-17](./invariants.md#i-17-push-は列単位で互換を保つキー省略は既存値を保持する)）。

| 送り方 | 意味 |
|---|---|
| キーが無い | その列は触らない。サーバーの既存値がそのまま残る |
| キーがあり値が `null` | その列を `NULL` にクリアする |

旧バージョンのクライアントが知らない列を省略しても、その列は消えません。値を消したいときは
省略ではなく明示的に `null` を送ります。省略された列名は Worker のログに記録されます（列名のみ）。
