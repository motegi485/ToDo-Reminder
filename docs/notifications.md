...
- **停止フラグはローカル通知にも効きます**（2026-08-31 に追加）。`fireDueLocalNotifications()` と
  `getNotificationDelivery()` の入口で見ます。停止処理はブラウザ購読を解除するため、
  **正常に停止できた端末ほど下のフォールバック条件（購読が無い）を満たしてしまい**、
  アプリを開いている間ローカル通知が出ていました（[invariants.md I-13](./invariants.md#i-13-push-通知とローカル通知は排他)）。
- 停止処理（`unsubscribePush`）の順序は **フラグを立てる → ブラウザ側の購読解除 →
  サーバー側の行削除**。この順序なら、サーバー側の削除が失敗しても、その endpoint は
  以後 404/410 を返すので cron の失効判定が行を自動的に消します。
- **2 つの解除は独立に試します**（2026-08-31 に是正）。同じ `try` に入れていたため、
  ブラウザ側の解除が例外になるとサーバー側の削除を試さないまま抜け、**購読行が残ったまま
  Push が届き続ける**経路がありました。ブラウザ側だけ失敗して残る「サーバー行は消えたが
  端末の購読は生きている」状態は、送り先が無いので通知は出ません。再開操作は同じ endpoint で
  購読し直すため、そこから復帰できます。
...
失効購読の削除ぶんの予約は、**実際に送信する購読数**から取ります（2026-08-31 に是正）。失効判定は
送信結果からしか出ないためで、従来のように取得した全購読数から取ると、同期コード数が多い構成で
「失効が 1 件も無いのに全候補を毎分見送る」状態になり得ました
（[invariants.md I-10b](./invariants.md#i-10b-cron-の候補処理は-d1外部-fetch-の予算で見送る)）。
...
`Notification.permission` は**ブラウザが許可しているか**しか表しません。サーバーに購読行が
無ければ Push は 1 通も届きませんが、許可状態は `granted` のままです。設定画面が「許可済み」と
だけ出していると、**通知が来ないのに来ると信じてしまう**状態を作ります。

`src/lib/notifyClient.ts` の `getNotificationDelivery()` が実効的な配信経路を返し、
`NotificationStatus` が「この端末で受け取れます」／「アプリを開いている間だけ通知します」＋
サーバー登録のやり直し／「通知は届きません」を出し分けます。

| 戻り値 | 意味 |
|---|---|
| `{ kind: 'push' }` | サーバー Push が届く（閉じていても通知される） |
| `{ kind: 'local_only', reason: 'unconfirmed' }` | ブラウザ購読はあるがサーバー登録が未確認 |
| `{ kind: 'local_only', reason: 'not_subscribed' }` | まだ購読していない |
| `{ kind: 'local_only', reason: 'no_vapid' }` | `VITE_VAPID_PUBLIC_KEY` 未設定のビルド |
| `{ kind: 'none', reason: 'no_sw' \| 'no_registration' }` | どの経路でも届かない |

**判定条件は `offlineNotify.ts` の分岐と同一に保つこと。** あちらが「ローカル通知を出す」ケースが
ここでの `local_only` にちょうど対応します。ずらすと表示と実際の配信経路が食い違い、
いちばん直したかった嘘がそのまま残ります。

### リマインダー設定時にその場で許可を求める

`ReminderField` はトグルが ON のあいだ、通知が鳴らない状態なら注意文と導線を出します
（未設定＝「通知を許可」ボタン／拒否＝ブラウザ設定への案内／iOS の非 standalone＝
ホーム画面追加の案内と `MobilePwaGuide`）。これが無いと、未許可のままリマインダー付きタスクを
作っても **黙って何も起きません**。

**許可ダイアログは必ずユーザー操作の起点で出すこと。** トグル ON で自動要求すると、その場で
拒否された端末は以後アプリから許可を出し直せなくなります。

`MobilePwaGuide` は `open` / `onClose` を渡すと制御コンポーネントになり、起動時の自動表示は
行いません（渡さなければ従来どおり App.tsx で 1 回だけ自動表示）。制御時は「今後表示しない」を
出しません — 自分で開いたものに恒久的な抑止を提案するのは筋が違ううえ、押されると通知が
本当に必要になったときの案内まで失われます。
