...
### 既知の制約: `updated_at` が完了時刻を兼ねている

上の手順 2 の「完了時刻」と、Cron 側の「この周期に完了済みか」の判定
（`workers/cron/notify.ts` の `task.updated_at >= periodStartMs(...)`）は、どちらも
**`updated_at` を完了時刻として読んでいます**。しかし `updated_at` は**あらゆる編集**で進みます。

そのため「**周期が明けた後、復活処理が走る前にそのタスクを編集する**」と、`completed` のまま
`updated_at` だけが新しい周期に入り、クライアントもサーバーも「今周期は完了済み」と誤認します。
結果、**その周期ぶんの復活と通知が 1 回飛びます**（次の周期で自然回復）。

特に踏みやすいのが `renameProject` で、これは status を問わず該当プロジェクトの全行を更新するため、
`completed` の繰り返しタスクも巻き込みます。同一端末なら 30 秒 tick で復活するので窓は狭いですが、
**他端末で名前を変えた場合**はサーバー行が `completed` + 新しい `updated_at` のまま残るため、
その周期を恒久的に落とします。

**解消には同期対象の完了専用列（例: `last_completed_at`）が要ります。** D1 のスキーマ変更と、
既存 `completed` 行のバックフィル方針（`NULL` のとき `updated_at` で代用するのか、一度だけ
全復活を許すのか）の決定を伴います。列を足す手順そのものは
